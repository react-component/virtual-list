...
const useHeightCache = () => {
  ...
  const getHeightByKey = useCallback((key: Key): number | undefined=> {
    return cacheRef.current.get(key);
  }, [cacheRef])
  const udpateHeightByKey = useCallback((key: Key, height: number) => {
...
const useWidthCache = () => {
  const cacheRef = useRef(new Map<Key, number>());
  const getWidthByKey = useCallback((key: Key): number | undefined=> {
    return cacheRef.current.get(key);
  }, [cacheRef])
  const udpateWidthByKey = useCallback((key: Key, width: number) => {
...
const useCollectRAF = () => {
  ...
  const updateCollectRAF = useCallback((rafId: number)=> {
...
function useElementCache() {
  ...
  const forEachElement = useCallback((forEachFn)=> {
    elementRef.current.forEach(forEachFn);
  }, [elementRef]);

  const getElementByKey = useCallback((key: Key) => {
    return elementRef.current.get(key);
  }, [elementRef]);

  
  const updateElementByKey = useCallback((key: Key, element: HTMLElement) => {
    return elementRef.current.set(key, element);
  }, [elementRef]);

  const deleteElementByKey = useCallback((key: Key) => {
...
export default function useInitCache<T>(
...
): [(item: T, instance: HTMLElement) => void, () => void, (key: Key) => number, number] {
  const [updatedMark, setUpdatedMark] = useState(0);
  const [getElementByKey, forEachElement, updateElementByKey, deleteElementByKey] = useElementCache();
  ...
  }, [isHorizontalMode, forEachElement, getRectSizeByKey, setRectSizeByKey, cancelRAF, updateCollectRAF]);

  const updateElement = useCallback((item: T, element: HTMLElement)=> {
  ...
  }, [getKey, getElementByKey, updateElementByKey, collectRectSize, deleteElementByKey, onItemAdd, onItemRemove]);
